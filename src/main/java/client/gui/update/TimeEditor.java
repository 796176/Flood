...
/**
 * TimeEditor is an editor for {@link JSpinner} whose model is {@link SpinnerTimeModel}.
 * The set of displayed characters is limited to digits and a colon ( : ).
 */
public class TimeEditor extends JSpinner.DefaultEditor {
	/**
	 * Constructs a TimeEditor editor that supports displaying and editing the value of a SpinnerTimeModel with a JFormattedTextField.
	 * @param spinner spinner whose model this editor will monitor
	 */
	public TimeEditor(JSpinner spinner) {
		super(spinner);

		spinner.getModel().addChangeListener(changeEvent -> {
			if (changeEvent.getSource() instanceof SpinnerTimeModel stm) {
				getTextField().setValue(stm.getValue());
			}
		});
